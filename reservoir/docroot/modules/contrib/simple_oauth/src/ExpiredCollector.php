...
class ExpiredCollector {
  ...
  public function collectForAccount(AccountInterface $account) {
    ...
    // default user. But to use that, simple_oauth_extras must be installed, so
    // ensure to only do it when appropriate.
    $stored_field_names = array_keys($this->clientStorage->getFieldStorageDefinitions());
    if (in_array('user_id', $stored_field_names, TRUE)) {
